...
describe('Exchange', () => {
	...
	describe('Depositing Tokens', () => {
		...
		describe('Success', () => {
			...
			it('emits a Deposit event', async () => {
				...
				const event = result.events[1]  // Approval event is [0]
				//console.log(event)    // to see the event emitted
		...
		describe('Failure', () => {
			it('fails when no tokens are approved', async () => {
				// we have not approved any tokens yet...
	...
	describe('Withdrawing Tokens', () => {
		...
		describe('Success', () => {
			...
			it('emits a Withdraw event', async () => {
				//console.log(result)   // to see entire logged tx from above
				const event = result.events[1]  // Approval is [0] ??
				//console.log(event)    // to see the event emitted
	...
	describe('Making orders', async () => {
		...
		describe('Success', async () => {
			...
			it('emits an Order event', async () => {
				...
				expect(args.timestamp).to.at.least(1)		// hard to test exact time
			})
		})
	...
	describe('Order Actions', async () => {
		...
		beforeEach(async () => {
			// first set up an order for the test
			// approve token
			transaction = await token1.connect(user1).approve(exchange.address, amount)
			result = await transaction.wait()
			// deposit token
